...
document.getElementById("theme-menu-btn").addEventListener("click", () => {
   ...
   durMenu.classList.remove("grid");
   durMenu.classList.add("hidden");
   typeMenu.classList.remove("grid");
   typeMenu.classList.add("hidden");
});

themes.forEach((thm) => {
   thm.addEventListener("click", () => {
      let themeData = thm.dataset.theme;
      ...
      let thmMenu = document.getElementById("theme-menu");
      thmMenu.classList.remove("grid");
      thmMenu.classList.add("hidden");
   });
});
document.getElementById("timeleft").addEventListener("click", async () => {
   try {
      const response = await fetch("./data.json");
      const list = await response.json();
      const found = list.englishTwentyK;

      // Assuming found is an array of strings
      const fck = found.find((word) => word === "fuck");
      console.log(fck);
   } catch (error) {
